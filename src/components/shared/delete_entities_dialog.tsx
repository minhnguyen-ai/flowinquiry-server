...
interface DeleteEntitiesDialogProps<TEntity extends Record<string, any>>
  extends React.ComponentPropsWithoutRef<typeof Dialog> {
  entities: Row<TEntity>["original"][];
  showTrigger?: boolean;
  onSuccess?: () => void;
  deleteEntitiesFn: (ids: string[]) => Promise<{ error?: string }>; // Pass a function to delete entities
  entityName: string; // Pass the entity name for the UI text
}

export function DeleteEntitiesDialog<TEntity extends Record<string, any>>({
  entities,
  showTrigger = true,
  onSuccess,
  deleteEntitiesFn,
  entityName,
  ...props
}: DeleteEntitiesDialogProps<TEntity>) {
  ...
  function onDelete() {
    startDeleteTransition(async () => {
      ...
      const { error } = await deleteEntitiesFn(ids);
